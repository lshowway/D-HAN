...
from dataset import ReadingNEWSTrain, ReadingNEWSTest
from torch.utils.data import DataLoader
from transformers import get_linear_schedule_with_warmup, AdamW

# from dna_CNN import Tacnn
# from dna_trans import Tacnn
...
# from dna_all_trans_timeEmbedding import Tacnn
...
class Recommender():
    ...
    def _initialize(self, interactions):
        ...
        self._optimizer = AdamW(optimizer_grouped_parameters, lr=self._learning_rate, weight_decay=self.weight_decay)

        self.scheduler = get_linear_schedule_with_warmup(
            self._optimizer, num_warmup_steps=0.0, num_training_steps=6140*25)
    ...
    def fit(self, train_loader, test_loader):
        for epoch_num in range(0, self._n_iter): # epoch
            ...
            t2 = time.time()
            if (epoch_num) % 1 == 0:
                HR, NDCG = self.test_epoch(test_loader, ks=10)
                output_str = "Epoch %d [%.1f s]\ttrain loss=%.4f," \
                             "test, HR@1=%.4f,HR@2=%.4f,HR@3=%.4f,HR@4=%.4f,HR@5=%.4f,HR@6=%.4f,HR@7=%.4f,HR@8=%.4f,HR@9=%.4f,HR@10=%.4f, " \
    ...
    def train_epoch(self,train_loader):
        ...
        epoch_loss = 0
        # count = 0
        # for batch_idx, (news_hist_tensor, news_hist_element_tensor, news_hist_id_tensor,
        #                 news_cand_tensor, news_cand_element_tensor, news_cand_id_tensor,
        #                 news_neg_tensor, news_neg_element_tensor, news_neg_id_tensor,
        #                 user_tensor, hitory_time, cadidate_time) in enumerate(train_loader): # batch
        for batch_idx, (news_hist, news_hist_ele, news_hist_id,
                        target_news, target_ele, target_news_id,
                        user, history_time, target_time,
                        candidate_news, candidate_ele, candidate_news_id) in enumerate(train_loader):
            ...
            #                               user.to(device),
            #                               history_time.to(device),
            #                               target_time.to(device))  # 已验证：torch.Size([batch, 1])
            ...
            target_prediction,  items_all, loss_ns = self._net(news_hist.to(device),
                                  ...
                                  target_time.to(device),
                                          True,
                                          candidate_news_id.to(device),
                                          candidate_news.to(device),
                                          candidate_ele.to(device),
                                          )
            # DNS
            ...
            # 反向传播
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self._net.parameters(), max_norm=20.0)
    ...
    def test_epoch(self, test_loader, ks):
        self._net.eval()
        hr = [0] * ks
        ndcg = [0] * ks
        count = 0
...
def get_args():
    ...
    # private / cert
    parser.add_argument('--train_root', type=str, default='../cert/userSeq_train.txt')
    parser.add_argument('--test_root', type=str, default='../cert/userSeq_test.txt')
    parser.add_argument('--content_word', type=str, default='../cert/newsid_word.txt')
    parser.add_argument('--content_word_w', type=str, default='../cert/newsid_word.vec')
    parser.add_argument('--embed_root', type=str, default='../../zh.64/')
    parser.add_argument('--element_root', type=str, default='../cert/newsid_embed_private.txt')
    parser.add_argument('--element_root_w', type=str, default='../cert/newsid_embed_private.vec')
    parser.add_argument('--generate_seq', type=str, default='../cert/generateSequence/')
    parser.add_argument('--doc2vec_model', type=str, default='../cert/doc2vec_private')
    # private
    ...
    parser.add_argument('--interval_or_abs', type=str, default='interval')
    parser.add_argument('--embedding_noise_prob', type=float, default=0.2)
...
if __name__ == '__main__':
    ...
    test_candidate = generate_candidate_train(train)


    train_dataset = ReadingNEWSTrain(sequences=train.sequences, targets=train.sequences.targets,
                                targets_time=train.sequences.targets_time,
                                newsMap=newsMap, elementsMap=elementsMap,
                                usercandidate=test_candidate, negs=config.neg_samples, senK=config.senK,)
    test_dataset = ReadingNEWSTest(sequences=train.test_sequences, targets=test.item_ids,
